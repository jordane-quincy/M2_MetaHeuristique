                ...
                for (j = 1; j <= mkp->n; j++) {
                    ...
                    if (j != i && copieS->x[j] == 0 && isAddPossible(mkp, copieS, j) && mouvementNotInTabouList(i, j, listTabou, copieS->objValue + mkp->a[0][j])) {
                        if (mkp->a[0][j] > mkp->a[0][i]) {
                            ...
                            //(normalement elle est forc�ment am�liorante puisqu'on ajoute uniquement les objets avec une valeur supérieur à l'objet qu'on a enlevé)
...
int main(int argc, char *argv[]) {
    ...
    //sAmeliorante = NULL;
    sAmeliorante = parcoursVoisin(mkp, sol, 1, sol, listTabou, 0, 0);
